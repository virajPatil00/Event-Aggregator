...
router.get('/recommendations', requireAuth, async (req: AuthRequest, res) => {
	const pref = await prisma.preference.findUnique({ where: { userId: req.user!.userId } });
	const where: any = { isPublished: true, startTime: { gte: new Date() } };
	if (pref?.categories) where.category = { in: pref.categories as string[] };
	if (pref?.departments) where.department = { in: pref.departments as string[] };
